...
const DropDown = ({ listItem, className, setStateCallback }) => {
  ...
  return (
      <div className='relative'>
        ...
        {listItem.map(item => (
            <li key={item.id} className='cursor-pointer rounded-lg h-20 px-4 py-2 hover:bg-slate-300/30 transition border-b border-slate-200/50 flex items-center'>{item.title}</li>
      ))}
    </ul>
     </div>
  )
